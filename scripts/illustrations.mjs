...
import { W, H, r1, rng, inCircle, shadow, plate, bowl, skillet, sheetPan, napkin, fork, leaf, sprig, lemonSlice, lemonWedge, garlicClove, thigh, pea, cube, grain, scatter, floret, beefStrip, bean, egg, chickpea, potato, noodle, stick } from './lib/illo-shapes.mjs';
import { KINDS, bitsAt } from './lib/illo-kinds.mjs';
import { SPECS, EXTRA } from './lib/illo-specs.mjs';
...
// Template-based illustrations: one spec line each (scripts/lib/illo-specs.mjs).
for (const [id, [kind, bg, title, opts]] of Object.entries(SPECS)) {
  if (COMPOSE[id]) throw new Error(`duplicate illustration ${id}`);
  COMPOSE[id] = EXTRA[id] ? (rand) => EXTRA[id](rand, KINDS, bitsAt) : (rand) => KINDS[kind](rand, opts);
  BACKGROUNDS[id] = bg;
  TITLES[id] = title;
}
