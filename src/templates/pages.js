...
<section class="hero">
  <div class="wrap">
    ...
    <ul class="hero-links">
      ...
      <li><a class="pill-link" href="/shopping-list/">Shopping list</a></li>
    </ul>
  </div>
</section>
${sections.map(
...
export function recipeJsonLd(r, site) {
  ...
  const data = {
    ...
    image: [img('1x1'), img('4x3'), img('16x9')],
...
<article class="recipe" data-recipe-id="${r.id}">
  <div class="wrap">
    ...
    <header class="recipe-head">
      ...
      <figure class="recipe-img"><img src="/images/recipes/${r.id}.svg" width="400" height="300" alt="${r.image.alt}" decoding="async" fetchpriority="high"></figure>
    </header>
...
<form class="wrap search-form" action="/search/" method="get" role="search" data-search-form>
  ...
  <div class="filters needs-js">
    <fieldset class="chip-set"><legend>Meal</legend><div class="chips">${chip('meal', 'breakfast', 'Breakfast')}${chip('meal', 'lunch', 'Lunch')}${chip('meal', 'dinner', 'Dinner')}${chip('meal', 'dessert', 'Dessert')}${chip('meal', 'side', 'Sides')}</div></fieldset>
    <fieldset class="chip-set"><legend>Diet and style</legend><div class="chips">${chip('diet', 'vegetarian', 'Vegetarian')}${chip('diet', 'vegan', 'Vegan')}<input type="checkbox" name="quick" value="1" id="f-quick"><label for="f-quick">Quick (30 min or less)</label>${chip('tag', 'one-pan', 'One-pan')}${chip('tag', 'make-ahead', 'Make-ahead')}</div></fieldset>
...
<script type="application/json" id="search-data">${jsonScript(recipes.map((r) => searchEntry(r, ctx.vocab)))}</script>`;
  return layout(ctx, {
    path: '/search/',
...
export function discoverPage(ctx) {
  ...
  for (const k of usedKeys) {
    const v = vocab[k];
    miniVocab[k] = { one: v.one, family: v.family, syn: v.syn, pantry: v.pantry };
  }
...
<script type="application/json" id="discover-data">${jsonScript({ entries: recipes.map((r) => searchEntry(r, vocab)), vocab: miniVocab })}</script>`;
  return layout(ctx, {
...
<div class="wrap prose">
  ...
  <ul>
    ...
    <li><strong>Checklists and shopping lists</strong> are saved in your browser, never on a server.</li>
  </ul>
