...
import { initPage, loadData, debounce } from './core.js';
...
const TAGS = ['one-pan', 'make-ahead', 'freezer-friendly'];
const LABELS = ['dairy-free', 'egg-free', 'gluten-free', 'nut-free'];
...
function fromURL() {
  ...
  return {
    ...
    labels: p.getAll('label').filter((l) => LABELS.includes(l)),
    quick: p.get('quick') === '1',
...
function toForm(s) {
  ...
  for (const box of form.querySelectorAll('input[type="checkbox"]')) {
    ...
    box.checked = name === 'meal' ? s.meals.includes(value) : name === 'diet' ? s.diets.includes(value) : name === 'tag' ? s.tags.includes(value) : name === 'label' ? s.labels.includes(value) : name === 'quick' ? s.quick : false;
  }
...
function toURL(s) {
  ...
  for (const l of s.labels) p.append('label', l);
...
function run(s) {
  ...
  const filtered = s.meals.length || s.diets.length || s.tags.length || s.labels.length || s.quick || s.maxTime || s.cuisine || s.sort || s.q.trim();
...
loadData('index.json')
  ...
  .catch(() => {
    note.hidden = false;
    note.textContent = 'Search couldn’t load. Check your connection and reload the page.';
  });
...
reset.addEventListener('click', () => {
  toForm({ q: '', meals: [], diets: [], tags: [], labels: [], quick: false, maxTime: null, cuisine: '', sort: '' });
  update();
});
