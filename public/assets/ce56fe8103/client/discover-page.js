...
import { initPage, loadData, esc, store } from './core.js';
...
initPage();
let data = { entries: [], vocab: {} };
try {
  const [entries, vocab] = await Promise.all([loadData('index.json'), loadData('vocab.json')]);
  data = { entries, vocab };
} catch {
  const note = document.querySelector('[data-unmatched]');
  note.hidden = false;
  note.textContent = 'This tool couldn’t load. Check your connection and reload the page.';
}
